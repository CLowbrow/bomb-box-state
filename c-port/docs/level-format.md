...
The portable C17 core owns the authoritative decoder, encoder, and structural validator. Generic
...
use a C++ runtime, a filesystem, browser APIs, or Unreal. JSON is passed as an in-memory byte
string. Native hosts may bypass JSON by constructing the typed level structs declared in
`game_rules/c_api.h`.

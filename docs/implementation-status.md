...
| --- | --- | --- |
| Native debug | `cmake --preset native-debug`; `cmake --build --preset native-debug`; `ctest --preset native-debug --output-on-failure` | Passed: 93 of 93 tests: 79 behavior cases, 5 C ABI boundary cases, 5 focused unit cases, 2 cross-adapter contract runners, and 2 production consumer/header smokes. |
